...
#include <Servo.h>
#include <ESP8266WebServer.h> // 引入WebServer库


/* 连接您的WIFI SSID和密码 */
#define WIFI_SSID         "ImmortalWrt-2.4G"
#define WIFI_PASSWD       "2901325922"
...
WiFiServer wifiServer(8266); // 创建WiFi服务器实例
...
void handleRoot() {
    ...
    html += "<button onclick=\"sendCommand('a')\">Turn Left</button>";
    html += "<button onclick=\"sendCommand('b')\">Forward</button>";
    html += "<button onclick=\"sendCommand('c')\">Turn Right</button>";
    html += "<button onclick=\"sendCommand('d')\">Move Left</button>";
    html += "<button onclick=\"sendCommand('e')\">Backward</button>";
    html += "<button onclick=\"sendCommand('f')\">Move Right</button>";
    html += "<button onclick=\"sendCommand('g')\">Standby</button>";
    html += "<button onclick=\"sendCommand('h')\">Say Hi</button>";
    html += "<button onclick=\"sendCommand('i')\">Push Up</button>";
    html += "<button onclick=\"sendCommand('j')\">Lie</button>";
    html += "<button onclick=\"sendCommand('k')\">Fighting</button>";
    html += "<button onclick=\"sendCommand('l')\">Sleep</button>";
    html += "<button onclick=\"sendCommand('m')\">Dancing 1</button>";
    html += "<button onclick=\"sendCommand('n')\">Dancing 2</button>";
    html += "<button onclick=\"sendCommand('o')\">Dancing 3</button>";
    html += "<button onclick=\"sendCommand('p')\">Zero Position</button>";
    html += "<script>function sendCommand(cmd) { var xhttp = new XMLHttpRequest(); xhttp.open('GET', '/' + cmd, true); xhttp.send(); }</script>";
    html += "</body></html>";
    server.send(200, "text/html", html);
}

void handleCommand() {
    String cmd = server.uri().substring(1);
    ActonFlag = cmd.charAt(0);
